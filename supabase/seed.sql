...
-- ============================================================================
-- EXTRACTION CONFIGS (Seed configs for common job posting formats)
-- ============================================================================

-- Schema.org JobPosting JSON-LD config
-- Matches pages with <script type="application/ld+json"> containing "JobPosting"
-- Works for: Netflix, LinkedIn, Google Jobs, and many other sites using structured data
INSERT INTO extraction_configs (name, version, match_patterns, match_hash, extract_rules)
VALUES (
  'Schema.org JobPosting (JSON-LD)',
  1,
  -- Match patterns: Look for JSON-LD script tag with JobPosting type
  '[
    {"patternType": "CssExists", "selector": "script[type=\"application/ld+json\"]", "contentContains": "JobPosting"}
  ]'::jsonb,
  -- Pre-computed hash for this pattern set (generated by ExtractionConfig.computeMatchHash)
  '2a2f7685994835fdfd54fff44199c4c72e664e9d4780b63eb2568f6f59c28b14',
  -- Extract rules map field names to extraction rules
  '{
    "title": [
      {"source": "JsonLd", "path": "$.title"},
      {"source": "JsonLd", "path": "$.name"}
    ],
    "description": [
      {"source": "JsonLd", "path": "$.description", "transforms": ["HtmlDecode"]}
    ],
    "companyName": [
      {"source": "JsonLd", "path": "$.hiringOrganization.name"},
      {"source": "JsonLd", "path": "$.hiringOrganization"}
    ],
    "location": [
      {"source": "JsonLd", "path": "$.jobLocation.address.addressLocality"},
      {"source": "JsonLd", "path": "$.jobLocation.address"},
      {"source": "JsonLd", "path": "$.jobLocation.name"},
      {"source": "JsonLd", "path": "$.jobLocation"}
    ],
    "salaryMin": [
      {"source": "JsonLd", "path": "$.baseSalary.value.minValue", "transforms": ["ParseNumber"]},
      {"source": "JsonLd", "path": "$.baseSalary.value", "transforms": ["ParseNumber"]},
      {"source": "JsonLd", "path": "$.estimatedSalary.value.minValue", "transforms": ["ParseNumber"]}
    ],
    "salaryMax": [
      {"source": "JsonLd", "path": "$.baseSalary.value.maxValue", "transforms": ["ParseNumber"]},
      {"source": "JsonLd", "path": "$.estimatedSalary.value.maxValue", "transforms": ["ParseNumber"]}
    ],
    "salaryCurrency": [
      {"source": "JsonLd", "path": "$.baseSalary.currency"},
      {"source": "JsonLd", "path": "$.estimatedSalary.currency"}
    ],
    "jobType": [
      {"source": "JsonLd", "path": "$.employmentType"}
    ],
    "experienceLevel": [
      {"source": "JsonLd", "path": "$.experienceRequirements"},
      {"source": "JsonLd", "path": "$.qualifications"}
    ],
    "educationLevel": [
      {"source": "JsonLd", "path": "$.educationRequirements.credentialCategory"},
      {"source": "JsonLd", "path": "$.educationRequirements"}
    ],
    "qualifications": [
      {"source": "JsonLd", "path": "$.qualifications"},
      {"source": "JsonLd", "path": "$.skills"}
    ],
    "responsibilities": [
      {"source": "JsonLd", "path": "$.responsibilities"}
    ],
    "benefits": [
      {"source": "JsonLd", "path": "$.jobBenefits"}
    ],
    "applicationUrl": [
      {"source": "JsonLd", "path": "$.url"},
      {"source": "JsonLd", "path": "$.directApply"}
    ],
    "postedDate": [
      {"source": "JsonLd", "path": "$.datePosted"}
    ],
    "closingDate": [
      {"source": "JsonLd", "path": "$.validThrough"}
    ],
    "isRemote": [
      {"source": "JsonLd", "path": "$.jobLocationType"},
      {"source": "JsonLd", "path": "$.applicantLocationRequirements"}
    ],
    "category": [
      {"source": "JsonLd", "path": "$.industry"},
      {"source": "JsonLd", "path": "$.occupationalCategory"}
    ]
  }'::jsonb
);
